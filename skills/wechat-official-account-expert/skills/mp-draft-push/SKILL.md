...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `title` | string | [合格] | 文章标题（不超过 64 字节，约 21 个中文字符） |
| `digest` | string | [合格] | 文章摘要（显示在分享卡片上） |
| `content_html` | string | [合格] | 文章正文 HTML（使用内联样式） |
| `cover_image_path` | string | [拦截] | 封面图本地路径（如不提供则用兜底 URL 或无封面） |
...
> [注意] 平台**不保存** AppSecret，需自行妥善保管。忘记只能重置生成新的。
...
2. 若 `gen_image` 不可用，检查环境变量 `DEFAULT_COVER_URL`：有值则下载到 `/tmp/wechat_cover_default.png` 再上传
3. 以上均不可用：`thumb_media_id` 留空（草稿不含封面）
